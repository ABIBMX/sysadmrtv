		...
		if(isset($_POST['accion']))
		{
			switch($_POST['accion'])
			{
				case 'editar':
					...
					//$query = "update depositos set id_banco='".addslashes(strtoupper($_POST['banco']))."',monto='".addslashes(strtoupper($_POST['monto']))."',fecha='".addslashes(strtoupper($_POST['fechas']))."'  where id_deposito='".addslashes($_POST['id'])."'";
				...
				case 'agregar':
					...
					if(addslashes($_POST['comision'])==1){
						 $egreso = "insert into  egresos (id_egreso,id_tipo_egreso,id_empleado,id_estado_transaccion,id_comprobante,monto,fecha,referencia) 
							...
							if(mysqli_query($conexion,$egreso)){
								...
								insertarCaja($_SESSION['tuvision_id_sucursal'],addslashes($_POST['monto']),2, 0,$concepto_caja[0],NULL,strtoupper($_POST['r_comprobante']));
					...
					if(mysqli_query($conexion,$query))
					{
						...
						insertarCaja($_SESSION['tuvision_id_sucursal'],addslashes($_POST['monto_dep']),3, 0,$concepto_caja[0],NULL,strtoupper($_POST['id_ref_dep']));
					...
					foreach($_POST as $variable => $valor)
					{
						if($variable != "selector")
						{
							if($variable != "accion")
							{
								...
								$query_cancelar = "UPDATE depositos set id_estado_transaccion='2' WHERE id_deposito='".addslashes($variable)."' and id_empleado_deposita in (select id_empleado from empleados  where id_sucursal='".$_SESSION['tuvision_id_sucursal']."')";
								if(mysqli_query($conexion,$query_cancelar)){
									$bandera = true;
									$query_cancelar2 = "UPDATE egresos set id_estado_transaccion='2' where id_egreso=(select comision_id_egreso from depositos where id_deposito='".addslashes($variable)."') and id_empleado_autorizo in (select id_empleado from empleados  where id_sucursal='".$_SESSION['tuvision_id_sucursal']."')";
		...
		<td colspan="3">
			...
			<table class="datagrid" width="100%" border="0" cellspacing="0">
				...
				if(isset($_POST['banco'])){
						...
						if($_POST['sucursal']!="")
						{
							$add_query .= " and id_empleado_deposita in (select id_empleado from empleados where id_sucursal='".$_SESSION['tuvision_id_sucursal']."') ";
						}
					...
					monto,
					fecha from depositos d where d.id_estado_transaccion='1' and id_empleado_deposita in (select id_empleado from empleados  where id_sucursal='".$_SESSION['tuvision_id_sucursal']."') ".$add_query;
