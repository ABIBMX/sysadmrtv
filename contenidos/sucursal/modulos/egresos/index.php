					...
					foreach($_POST as $variable => $valor)
					{
						if($variable != "selector")
						{
							if($variable != "accion")
							{
								...
								if($num ==0)
								$query_eliminar = "UPDATE egresos SET id_estado_transaccion='2' where id_egreso ='".addslashes($variable)."' and id_empleado in (select id_empleado from empleados where id_sucursal='".$_SESSION['tuvision_id_sucursal']."')";
			...
			<table class="datagrid" width="100%" border="0" cellspacing="0">
				...
				if(addslashes($_POST['egre'])){
					...
					$add_query="";
						if($_POST['suc']!="null")
						{
							$add_query .= " and id_empleado in (select id_empleado from empleados where id_sucursal='".$_SESSION['tuvision_id_sucursal']."') ";
						}
					...
					$query = "select id_egreso,(select descripcion from cat_tipo_egreso where id_tipo_egreso=e.id_tipo_egreso),(select concat(nombre,' ',apellido_paterno,' ',apellido_materno) from empleados where id_empleado=e.id_empleado),monto,fecha,(select descripcion from estado_transaccion where id_estado_transaccion = e.id_estado_transaccion) from egresos e where  id_empleado in (select id_empleado from empleados where id_sucursal ='".$_SESSION['tuvision_id_sucursal']."') ".$add_query;
