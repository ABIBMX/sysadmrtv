...
<table border="0px" width="100%" style="color:#000000;font-size:12px">
					...
					if (isset($_POST['servicio']))
						foreach ($_POST['servicio'] as $valor) {
							...
							if (mysqli_query($conexion, $query2)) {
								...
								// 	$query_cliente = "select id_cliente from reporte_servicios where id_reporte='" . $reporte . "'";
								// 	$cliente = mysqli_query($conexion, $query_cliente);

								// 	//actualizar el tipo de servicio del cliente y su configuracion de internet

								// 	$query_estatus = "update configuracion_internet set estatus='ANTERIOR' where id_cliente='" . $cliente . "'";
								// 	mysqli_query($conexion, $query);

								// 	$query_tipo_servicio = "update clientes set tipo_contratacion='SERVICIO CANCELADO', configuracion_internet='NO NECESARIO' where id_cliente='" . $cliente . "'";
								// 	mysqli_query($conexion, $query_tipo_servicio);
    ...
    (
        ...
        WHERE id_sucursal = (
            SELECT id_sucursal
            FROM empleados
            WHERE id_empleado = re.id_empleado
        )
    ) AS nombre_sucursal,
    re.id_cliente,
    (
...
WHERE re.id_estatus_servicio != 3
AND re.id_empleado IN (
    SELECT id_empleado
    FROM empleados
    WHERE id_sucursal = '" . $_SESSION['tuvision_id_sucursal'] . "'
)";

						// Agregar la condición WHERE para $tservicio_query si tiene valor
