<?php
	...
	if($registro_ingreso[0]!="")
	{
		...
		for($j=0;$j<2;$j++)
		{
			...
			$pdf->addTextWrap (25,700+$y,80,8,"NO. CLIENTE:",'left');
			$pdf->addTextWrap (25,685+$y,80,8,"NOMBRE:",'left');
			$pdf->addTextWrap (25,670+$y,80,8,"FECHA DE PAGO:",'left');
			$pdf->addTextWrap (110,700+$y,470,8,"<b>".$id_cliente."</b>",'left');
			$pdf->addTextWrap (110,685+$y,470,8,"<b>".$nombre."</b>",'left');
			$pdf->addTextWrap (110,670+$y,470,8,"<b>".strtoupper(transformarFecha($fecha))."</b>",'left');
			$pdf->line(20,665+$y,590,665+$y);
			
			//TOTAL
			$pdf->addTextWrap (480,555+$y,80,8,"<b>TOTAL: $</b>",'left');
			$pdf->addTextWrap (500,555+$y,80,8,"<b>".number_format($monto_total, 2, '.', ',')."</b>",'right');		
			$pdf->line(20,570+$y,590,570+$y);
			$pdf->line(20,545+$y,590,545+$y);
			
			$pdf->addTextWrap (25,555+$y,450,8,transformarCantidadLetras($monto_total)." M. N.",'center');	
			
			
			
			//CONCEPTOS
			
			$pdf->addTextWrap (25,650+$y,80,8,"CONCEPTO:",'left');		
			$pdf->addTextWrap (480,650+$y,80,8,"MONTO: <b>$</b>",'left');
			$pdf->addTextWrap (110,650+$y,400,8,"<b>".$concepto_principal."</b>",'left');
			if($promocion_principal!="")
				$pdf->addTextWrap (260,650+$y,200,5,$promocion_principal." ".$porcentaje_principal." %",'right');
			$pdf->addTextWrap (500,650+$y,80,8,"<b>".number_format($monto_principal, 2, '.', ',')."</b>",'right');
			
			
			
			//$pdf->setLineStyle(1,'','',array(5));
			$pdf->line(20,640+$y,590,640+$y);
			$pdf->addTextWrap (25,625+$y,80,8,"ADICIONALES:",'left');	;
			...
			{
				$pdf->addTextWrap (110,$y_adicionales,400,8,"<b>".$conceptos_adicionales[$i]['descripcion']."</b>",'left');
				if($conceptos_adicionales[$i]['promocion']!="")
					$pdf->addTextWrap (260,$y_adicionales,200,5,$conceptos_adicionales[$i]['promocion']." ".$conceptos_adicionales[$i]['porcentaje']." %",'right');
				$pdf->addTextWrap (515,$y_adicionales,80,8,"<b>$</b>",'left');
				$pdf->addTextWrap (500,$y_adicionales,80,8,"<b>".number_format($conceptos_adicionales[$i]['monto'], 2, '.', ',')."</b>",'right');				
			...
			//OBSERVACCIONES
			$pdf->addTextWrap (25,535+$y,80,8,"OBSERVACIONES:",'left');	;
			$pdf->ezSetY(530+$y);
			$pdf->ezText("<b>".strtoupper($observaciones)."</b>",7,array('justification'=>'full'));
